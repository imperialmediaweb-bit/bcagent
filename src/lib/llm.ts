...
export function getProvider(): Provider | null {
  ...
  if (explicit === "openai") return hasOpenAI ? "openai" : null;
  if (explicit === "anthropic") return hasAnthropic ? "anthropic" : null;
  // Auto: prefer OpenAI (user-supplied key e mai des), fallback Anthropic
...
export async function streamCompletion(opts: StreamOptions): Promise<void> {
  ...
  throw new Error(
    "AI provider neconfigurat — setează OPENAI_API_KEY sau ANTHROPIC_API_KEY.",
  );
}
