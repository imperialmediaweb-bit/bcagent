...
import { ANAF_BATCH_SIZE, queryAnafBatch } from "@/modules/prospects";
  ...
  try {
    ...
    for (let i = 0; i < pending.length; i += ANAF_BATCH_SIZE) {
      ...
      for (const cui of batch) {
        ...
        if (!firm) {
          // Negăsit la ANAF → probabil radiat de mult; marcăm inactiv
          await db`
          ...
          await db`
            UPDATE prospects SET
              activ = ${firm.activ},
              tva = ${firm.tva},
              adresa = CASE WHEN ${firm.adresa ?? ""} <> '' THEN ${firm.adresa ?? ""} ELSE adresa END,
              updated_at = NOW()
            WHERE cui = ${cui}
          `;
