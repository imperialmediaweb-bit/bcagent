...
/**
 * FIRMA DEMO: un buton în /platform/setari creează (sau reface de la zero)
 ...
 * deconturi. Perfect pentru prezentări — toate panourile arată vii.
 */

const DEMO_NAME = "Demo Distribuție SRL";
const AGENTS = [
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const existing = await listOrgs({ search: DEMO_NAME, limit: 5 });
    for (const o of existing.orgs) {
      if (o.name === DEMO_NAME) await deleteOrg(o.id);
    }
    ...
    await createOrgUser(org.id, "demo@bcagent.ro", ownerPass, "Patron Demo", "owner");
    const managerPass = generatePassword();
    await createOrgUser(
      org.id,
      "manager.demo@bcagent.ro",
      managerPass,
      "Bogdan Manager",
      "manager",
    );
    ...
    await audit(auth.session.email, "demo.create", org.id);
    return Response.json({
      ok: true,
      org: { id: org.id, name: DEMO_NAME },
      owner: { email: "demo@bcagent.ro", password: ownerPass },
      manager: { email: "manager.demo@bcagent.ro", password: managerPass },
      agentLinks,
    });
  } catch (e) {
